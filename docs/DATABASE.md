...
First-time setup writes the total number of sheds to `Settings`, replaces opening shed rows in `Sheds`, stores opening bird stock in the `Birds` column, and records opening feed stock in `FeedStock` with the reference `PRIMARY_SETUP`.
